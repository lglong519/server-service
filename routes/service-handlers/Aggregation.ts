...
type totalWeek = { total?: number; week?: number[] };
const query = (req, res, next) => {
	...
	let accesses: totalWeek = {};
	let squats: totalWeek = {};
	let pressUps: totalWeek = {};
	let auditlogs: totalWeek = {};
	let expenses = {
		food: <week>{},
		general: <week>{},
		total: 0
...
const git = (req, res, next) => {
	...
	if (Date.now() - new Date(gitCache.entryDate).getTime() < 300000) {
		res.json(gitCache);
		return next();
	}
	_gitOnlineFecth(req, res, next);
...
function _processContributors (contributorsCacheName, options) {
	return localStorage.fetchItem(contributorsCacheName, false).then(result => {
		// 判断分支缓存时间
		if (Date.now() - new Date(result.entryDate).getTime() < 300000) {
			result.cache = true;
			return result;
		}
