...
var xxx = function xxx(s?,...ars:any[]) {     // internal dev/debug logging
...
const debug= require('debug')('server:bunyan');
 ...
 *    creation.
 */
function Logger(options, _childOptions?, _childSimple?):void {
    ...
    delete (<Options>fields).stream;
    delete (<Options>fields).level;
    delete (<Options>fields).streams;
    delete (<Options>fields).serializers;
    delete (<Options>fields).src;
    if (this.serializers) {
        this._applySerializers(fields);
    }
    if (!(<Options>fields).hostname && !self.fields.hostname) {
        (<Options>fields).hostname = os.hostname();
    }
    if (!(<Options>fields).pid) {
        (<Options>fields).pid = process.pid;
    }
